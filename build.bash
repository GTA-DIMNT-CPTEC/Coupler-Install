#!/bin/bash
# =============================================================================
# build.bash — Compila/instala o acoplador em três etapas, na ordem de
# dependência. NÃO baixa o sistema: assume o MONAN-Coupler já presente
# (COUPLER_ROOT). Para baixar + instalar de uma vez, use install.bash.
# INPE / CGCT / DIMNT — GT Acoplamento de Modelos
#
# USO:
#   bash build.bash [--from N] [--only N] [--help]
...
#
# Exemplos:
#   bash build.bash            # completa (1-2-3)
#   bash build.bash --from 2   # pula MONAN-A
#   bash build.bash --only 3   # (re)linka bin/esmApp
# =============================================================================
set -euo pipefail
...
# Este repositório (instalador) é SEPARADO do sistema acoplado. COUPLER_ROOT vem
# do ambiente (exportado pelo install.bash) ou assume ./MONAN-Coupler. Para baixar
# e instalar em um único comando, prefira:  bash install.bash
...
usage() {
  cat << 'EOF'
Uso: bash build.bash [--from N] [--only N] [--help]
